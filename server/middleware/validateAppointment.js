...
const validateEmail = (v) => (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v) ? null : "Invalid email format");

const validatePhone = (v) =>
  /^(\+972|0)?[5-9]\d{8}$/.test(v.replace(/[-\s]/g, "")) ? null : "Invalid phone number format";

const validateDate = (v) => {
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  return new Date(v) < today ? "Appointment date must be in the future" : null;
};

const validateTime = (v) =>
  /^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/.test(v) ? null : "Invalid time format. Use HH:MM";
...
export const validateStatusUpdate = (req, res, next) => {
  ...
  if (!validStatuses.includes(status)) {
    return next(createError(400, `Status must be one of: ${validStatuses.join(", ")}`));
  }
